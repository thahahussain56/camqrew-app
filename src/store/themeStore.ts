...
export const useThemeStore = create<ThemeState>((set, get) => ({
  ...
  toggleTheme: async () => {
    ...
    await AsyncStorage.setItem('@camqrew_theme', nextMode);
  },
  setTheme: async (mode: 'dark' | 'light') => {
    const nextColors = mode === 'dark' ? DarkTheme : LightTheme;
    set({ mode, colors: nextColors });
    await AsyncStorage.setItem('@camqrew_theme', mode);
  },
  loadTheme: async () => {
    try {
      const saved = (await AsyncStorage.getItem('@camqrew_theme')) || (await AsyncStorage.getItem('@camcrew_theme'));
